...
import org.sree.storage.cassandra.CassandraMode;

import com.datastax.driver.core.Cluster;
import com.datastax.driver.core.Session;

@Service(value = "cassandraAdminService")
public class CassandraAdminServiceImpl implements AdminService {
	...
	public boolean createKeyspace(String schemaName) {
		try {
			...
			session.execute("CREATE KEYSPACE " + schemaName
					+ " WITH replication "
	...
	@Override
	public boolean createColumnFamily(String keyspace, String tableName,
			String key, Map<String, String> columns) {
		try {
			if (cluster == null) {
				cluster = CassandraConnectionFactory.create();
			}
			...
			session.execute("CREATE TABLE " + tableName + " (" + key
					+ " varchar, idvalue uuid, " + s + ", PRIMARY KEY(" + key
					+ ",idvalue)) WITH comment='user information' "
